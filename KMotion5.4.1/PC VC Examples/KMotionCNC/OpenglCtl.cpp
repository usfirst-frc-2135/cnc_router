...
void COpenglCtl::OnMouseMove(UINT nFlags, CPoint point) 
{
	...
	if (m_Ortho && m_xRotation == 0.0f && m_yRotation == 0.0f && m_yRotation == 0.0f)
	{
		...
		s.Format(L"G Code Viewer  xy = %.4f %.4f",x,y);
	}
	else
	{
		s.Format(L"G Code Viewer");
	}
...
void COpenglCtl::OnPaint() 
{
	...
	if (FirstTime)
	{
		CStringA Ver = glGetString(GL_VERSION);  
		int v0, v1, v2, r;
		...
		if (r < 2 || v0*10+v1 < 33)
		{
			...
			{
				DisplayError = false;
				// OpenGL version is too low, warn user
				CString msg;
				msg.Format(L"OpenGL %s version too low.  3.3 or higher required", (CStringW)Ver);
				AfxMessageBox(msg);
			}
			return;
		}
...
void COpenglCtl::SetupOpenGL()
{
	...
	{
		CScreen* S = &TheFrame->GCodeDlg.Screen;
		DLG_CONTROL* Dlg = S->FindDlgControl(IDC_viewctl);
		if (Dlg)
		{
			CString c = Dlg->Colors;
			if (c != ";;;" && c != "")
			{
				int Color0 = S->SwapRGB(S->PartHex(0, c));
				glClearColor(GetRValue(Color0), GetGValue(Color0), GetBValue(Color0), 1.0f);
				CustomColor = true;
			}
		}
	}
