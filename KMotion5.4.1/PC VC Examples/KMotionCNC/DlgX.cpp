...
int CDlgX::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	if (CDialog::OnCreate(lpCreateStruct) == -1)
		return -1;
	int wx2, nMonitors = GetSystemMetrics(SM_CMONITORS);

	int wx = GetSystemMetrics(SM_CXSCREEN);
	int wy = GetSystemMetrics(SM_CYSCREEN);

	if (nMonitors > 1)
		wx2 = GetSystemMetrics(SM_CXVIRTUALSCREEN);
	else
		wx2 = wx;

	if (LastMoveX > wx2 - 60) LastMoveX = wx2 - 60;
	if (LastMoveY > wy  - 60) LastMoveY = wy  - 60;
