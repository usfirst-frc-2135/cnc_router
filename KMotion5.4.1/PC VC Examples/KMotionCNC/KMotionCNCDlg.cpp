...
void CKMotionCNCDlg::ServiceKFLOPCommands()
{
	...
	case PC_COMM_SET_RRO:
		...
		if (NewValue > 0.001 && NewValue < 1000.0)   // ignore if crazy number
		{
			...
			if (m_Rapid == 0)
			{
				w.Format(L"%.2f", m_FeedRateRapidValue);
				SetDlgItemText(IDC_FeedRateEdit, w);
				m_FeedSlider.SetPos(m_FeedRateRapidValue);
			}
			SetKFLOPCommandResult(0);
	...
	case PC_COMM_SET_SSO:
		...
		if (NewValue > 0.001 && NewValue < 1000.0)   // ignore if crazy number
		{
			...
			w.Format(L"%.2f",m_SpindleRateValue);
...
void CKMotionCNCDlg::SetBigValues(CDisplay *Disp0, CDisplay *Disp1, CDisplay *Disp2, CDisplay *Disp3, CDisplay *Disp4, CDisplay *Disp5, CDisplay *Disp6, CDisplay *Disp7, bool KMotionPresent)
{
	...
	SetBigValueColor(Disp6, 6, KMotionPresent, DisplayedEnc6);
	SetBigValueColor(Disp7, 7, KMotionPresent, DisplayedEnc7);
	...
	s.Format(L" X"+Format,x);
	KillMinusZero(s);
	Disp0->SetText(s);

	s.Format(L" Y"+Format,y);
	KillMinusZero(s);
	Disp1->SetText(s);

	s.Format(L" Z"+Format,z);
	KillMinusZero(s);
	Disp2->SetText(s);

	s.Format(L" A"+Format,a);
	KillMinusZero(s);
	Disp3->SetText(s);

	s.Format(L" B"+Format,b);
	KillMinusZero(s);
	Disp4->SetText(s);

	s.Format(L" C"+Format,c);
	KillMinusZero(s);
	Disp5->SetText(s);

	if (m_DialogFaceInUse == CUSTOM_DLG_FACE)
	{
		s.Format(L" U" + Format, u);
		KillMinusZero(s);
		Disp6->SetText(s);

		s.Format(L" V" + Format, v);
		KillMinusZero(s);
		Disp7->SetText(s);
	}
}
...
{
	m_Zplus2.EnableWindow(DetermineEnableDisableKeys());
	m_Zplus.EnableWindow(DetermineEnableDisableKeys());
	m_ZplusStep.EnableWindow(DetermineEnableDisableKeys());
	...
	m_BplusStep.EnableWindow(DetermineEnableDisableKeys());
	...
	m_Cplus.EnableWindow(DetermineEnableDisableKeys());
	...
	m_Down.EnableWindow(DetermineEnableDisableKeys());
