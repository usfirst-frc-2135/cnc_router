...
static void asm_parse_directive(TCCState *s1)
{
    ...
    case TOK_ASM_space:
        ...
        if (sec->sh_type != SHT_NOBITS) {
            sec->data_offset = ind;
            ptr = (unsigned char *)section_ptr_add(sec, size);
            memset(ptr, v, size);
        }
        ind += size;
        break;
...
static void tcc_assemble_inline(TCCState *s1, char *str, int len)
{
    ...
    bf->fd = -1;
    bf->buf_ptr = (unsigned char *)str;
    bf->buf_end = (unsigned char *)str + len;
    str[len] = CH_EOB;
...
static void subst_asm_operands(ASMOperand *operands, int nb_operands, 
                               int nb_outputs,
                               CTString *out_str, CTString *in_str)
{
                               ...
                               int is_output)
{
    ...
    if (tok != ':') {
        ...
        for(;;) {
            ...
            op->constraint = (char *)tcc_malloc(tokc.cstr->size);
            strcpy(op->constraint, (const char *)tokc.cstr->data);
            next();
            skip('(');
            gexpr();
            if (is_output) {
...
static void asm_instr(void)
{
    ...
    while (tok == TOK_STR) {
        ...
        cstr_cat(&astr, (const char *)tokc.cstr->data);
        next();
    }
    cstr_ccat(&astr, '\0');
    nb_operands = 0;
        ...
        if (tok == ':') {
            ...
            if (tok == ':') {
                ...
                for(;;) {
                    if (tok != TOK_STR)
                        expect("string constant");
                    asm_clobber(clobber_regs, (const char *)tokc.cstr->data);
                    next();
                    if (tok == ',') {
                        next();
                    } else {
    ...
    tcc_assemble_inline(tcc_state, (char *)astr1.data, astr1.size - 1);
