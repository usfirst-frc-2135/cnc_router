...
static void relocate_common_syms(void)
{
    ...
    for(sym = (Elf32_Sym *)symtab_section->data + 1; 
        ...
        if (sym->st_shndx == SHN_COMMON) {
            ...
            offset = (offset + align - 1) & (-((signed)align));
            sym->st_value = offset;
...
static void relocate_section(TCCState *s1, Section *s)
{
    ...
    for(rel = qrel;
        ...
        case R_C60HI16:
            /* put the high 16 bits of the absolute address */
			// add to what is already there
			
//			orig  =   ((*(int *)(ptr-4)) >> 7) & 0xffff;
//			orig |=  (((*(int *)(ptr  )) >> 7) & 0xffff) << 16;

//			*(int *)ptr = (*(int *)ptr & (~(0xffff << 7)) ) | ((((val+orig)>>16) & 0xffff) << 7);
            break;
        }
    }
...
static void build_got(TCCState *s1)
{
    ...
    add_elf_sym(symtab_section, 0, 4, ELF32_ST_INFO(STB_GLOBAL, STT_OBJECT), 
                s1->got->sh_num, "_GLOBAL_OFFSET_TABLE_", 0);
    ...
    if (do_bounds_check) {
        ...
        *ptr = 0;
        add_elf_sym(symtab_section, 0, 0, 
                    ...
                    bounds_section->sh_num, "__bounds_start", 0);
...
//    }
    ...
    add_elf_sym(symtab_section, 
                ...
                ELF32_ST_INFO(STB_GLOBAL, STT_NOTYPE),
                text_section->sh_num, "_etext",0);
    add_elf_sym(symtab_section, 
                data_section->data_offset, 0,
                ELF32_ST_INFO(STB_GLOBAL, STT_NOTYPE),
                data_section->sh_num, "_edata",0);
    add_elf_sym(symtab_section, 
                bss_section->data_offset, 0,
                ELF32_ST_INFO(STB_GLOBAL, STT_NOTYPE),
                bss_section->sh_num, "_end",0);
    /* add start and stop symbols for sections whose name can be
       expressed in C */
    for(i = 1; i < s1->nb_sections; i++) {
        s = s1->sections[i];

#if (DO_C67)  // for c67 do all the sections
...
int tcc_output_file(TCCState *s1, const wchar_t*filename)
{
    ...
    int ret;
    int *section_order;
    int shnum, i, phnum, file_offset, j, tmp, sh_order_index, k;
    unsigned long addr;
    Section *strsec, *s;
    Elf32_Phdr *phdr, *ph;
        ...
        if (!s1->static_link) {
            ...
            if (file_type == TCC_OUTPUT_EXE) {
                ...
                for(esym = (Elf32_Sym *)s1->dynsymtab_section->data + 1; 
                    ...
                    esym++) {
                    if (esym->st_shndx == SHN_UNDEF) {
                        name = (const char *)s1->dynsymtab_section->link->data + esym->st_name;
                        sym_index = find_elf_sym(symtab_section, name);
                        if (sym_index) {
                            sym = &((Elf32_Sym *)symtab_section->data)[sym_index];
    ...
    if (phnum > 0) {
        ...
        for(j = 0; j < 2; j++) {
            ...
            for(k = 0; k < 5; k++) {
                for(i = 1; i < s1->nb_sections; i++) {
...
#if (DO_C67)
					// force absolute load/run address for C67
					// if specified as options use them
					// otherwise use default for text and
					// put other unspecified right after
					...
					{
						if (data_sect_addr==0xffffffff)
						{
							Section *stext = FindSection(s1,".text");
							addr = stext->sh_addr + stext->data_offset;
					...
					{
						if (bss_sect_addr!=0xffffffff)
						{
							addr = bss_sect_addr;  // specified use it
						}
						else
						{
							Section *sdata = FindSection(s1,".data"); // unspecified, put after data
							addr = sdata->sh_addr + sdata->data_offset;
							addr = (addr + s->sh_addralign - 1) & ~(s->sh_addralign - 1);
						}
					}
#endif
        ...
        if (dynamic) {
            ...
            /* first plt entry */
            p = (unsigned char *)section_ptr_add(plt, 16);
            ...
            for(sym = (Elf32_Sym *)s1->dynsym->data + 1; 
                ...
                if (sym->st_shndx == SHN_UNDEF) {
                    if (type == STT_FUNC) {
                        ...
                        put32(p + 12, -((signed)(plt->data_offset)));
...
{ 
    ...
    /* read sections */
    shdr = (Elf32_Shdr *)load_data(fd, file_offset + ehdr.e_shoff, 
                     sizeof(Elf32_Shdr) * ehdr.e_shnum);
    sm_table = (SectionMergeInfo *)tcc_mallocz(sizeof(SectionMergeInfo) * ehdr.e_shnum);
    
    /* load section names */
    sh = &shdr[ehdr.e_shstrndx];
    strsec = (unsigned char *)load_data(fd, file_offset + sh->sh_offset, sh->sh_size);
    ...
    for(i = 1; i < ehdr.e_shnum; i++) {
        ...
        if (sh->sh_type == SHT_SYMTAB) {
            ...
            symtab = (Elf32_Sym *)load_data(fd, file_offset + sh->sh_offset, sh->sh_size);
            sm_table[i].s = symtab_section;

            /* now load strtab */
            sh = &shdr[sh->sh_link];
            strtab = (unsigned char *)load_data(fd, file_offset + sh->sh_offset, sh->sh_size);
        }
    }
    ...
    found:
        if (sh->sh_type != (unsigned)s->sh_type) {
        ...
        if (sh->sh_addralign > (unsigned)s->sh_addralign)
            s->sh_addralign = sh->sh_addralign;
    ...
    /* resolve symbols */
    old_to_new_syms = (int *)tcc_mallocz(nb_syms * sizeof(int));
    ...
    for(i = 1; i < nb_syms; i++, sym++) {
        ...
        sym_index = add_elf_sym(symtab_section, sym->st_value, sym->st_size, 
                                sym->st_info, sym->st_shndx, name, 0);
...
static int tcc_load_archive(TCCState *s1, int fd)
{
    ...
    for(;;) {
        ...
        /* align to even */
        size = (size + 1) & ~1;
        _lseek(fd, file_offset + size, SEEK_SET);
    }
    return 0;
}
...
{ 
    ...
    /* read sections */
    shdr = (Elf32_Shdr *)load_data(fd, ehdr.e_shoff, sizeof(Elf32_Shdr) * ehdr.e_shnum);
    ...
    for(i = 0, sh = shdr; i < ehdr.e_shnum; i++, sh++) {
        ...
        case SHT_DYNAMIC:
            ...
            dynamic = (Elf32_Dyn *)load_data(fd, sh->sh_offset, sh->sh_size);
            break;
        case SHT_DYNSYM:
            nb_syms = sh->sh_size / sizeof(Elf32_Sym);
            dynsym = (Elf32_Sym *)load_data(fd, sh->sh_offset, sh->sh_size);
            sh1 = &shdr[sh->sh_link];
            dynstr = (unsigned char *)load_data(fd, sh1->sh_offset, sh1->sh_size);
            break;
        default:
            break;
        }
    ...
    for(i = 0, dt = dynamic; i < nb_dts; i++, dt++) {
        if (dt->d_tag == DT_SONAME) {
            soname = (const char *)dynstr + dt->d_un.d_val;
        }
    }
    ...
    dllref = (DLLReference *)tcc_malloc(sizeof(DLLReference) + strlen(soname));
    dllref->level = level;
    ...
    for(i = 0, dt = dynamic; i < nb_dts; i++, dt++) {
        switch(dt->d_tag) {
        case DT_NEEDED:
            name = (const char *)dynstr + dt->d_un.d_val;
            for(i = 0; i < s1->nb_loaded_dlls; i++) {
                dllref = s1->loaded_dlls[i];
