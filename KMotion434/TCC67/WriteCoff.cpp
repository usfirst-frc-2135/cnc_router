...
int WriteCoff(TCCState *s1, const char *OutFile)
{
    ...
    file_hdr.f_magic = 0x00c2;             /* magic number */
    ...
    for(i = 1; i < s1->nb_sections; i++) 
	{
		...
		if (OutputTheSection(tcc_sect))
		{
			...
			if (coff_sec->s_nreloc> 0)
			{
				...
				file_pointer += coff_sec->s_nreloc * sizeof(reloc);
			}
		}
	}
    ...
    for(i = 1; i < s1->nb_sections; i++) 
	{
		...
		if (OutputTheSection(tcc_sect))
		{
			...
			if (coff_sec->s_nreloc> 0)
			{
				fwrite(tcc_sect->reloc, coff_sec->s_nreloc * sizeof(reloc), 1, f);
			}
		}
	}
...
#define ADD_UNDERSCORE 0
			...
			{
				if (pCoff_str_table - Coff_str_table + strlen(_name) > MAX_STR_TABLE-1)
...
//			name[0]=0;
			 ...
			 (csym.n_type == 0x6          && csym.n_sclass == 0x2))     // floats
		{
			...
			//   not necessarily a need for a token			ts = tok_alloc(name,strlen(name));
...
{
	ts_divi = tok_alloc("_divi",sizeof("_divi"));
	ts_divu = tok_alloc("_divu",sizeof("_divu"));
	ts_divf = tok_alloc("_divf",sizeof("_divf"));
